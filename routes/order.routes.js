...
router.route("/:orderId/complete").post(isLoggedIn, completeOrder);

router.route("/:orderId/cancel").put(isLoggedIn, cancelOrder);

// getting orders (rejected, canceled, approved, completed) based on user_id
router.route("/u/orders/approved").get(isLoggedIn, getApprovedOrdersUsers);

router.route("/u/orders/rejected").get(isLoggedIn, getRejectedOrdersUsers);

router.route("/u/orders/confirmed").get(isLoggedIn, getConfirmedOrderUsers);

router.route("/u/orders/canceled").get(isLoggedIn, getCanceledOrderUsers);

// getting orders (confirmed, confirmed) based on seller_id
router.route("/s/orders/confirmed").get(isLoggedIn, getConfirmedOrderSeller);

router.route("/s/orders/canceled").get(isLoggedIn, getCanceledOrderSeller);
